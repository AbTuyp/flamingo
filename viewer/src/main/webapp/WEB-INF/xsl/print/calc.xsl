...
<!-- tekent schaalstok dmv svg -->
    ...
    <xsl:template name="windrose">
        ...
        <xsl:param name="width" select="'2cm'"/>
        <xsl:param name="height" select="'2cm'"/>
        <xsl:param name="top" select="'1cm'"/>
        <xsl:param name="left" select="'1cm'"/>		
        <xsl:param name="millipoints" select="29000"/>
        <fo:block-container fox:transform="translate({$millipoints},{$millipoints}) rotate({$angle}) translate(-{$millipoints}, -{$millipoints})" absolute-position="absolute" top="{$top}" left="{$left}">
            <fo:block>
                <fo:external-graphic width="{$width}" height="{$height}" content-height="scale-to-fit" content-width="scale-to-fit" src="noordpijl.svg"/>
            </fo:block>
        </fo:block-container>
    </xsl:template>
</xsl:stylesheet>